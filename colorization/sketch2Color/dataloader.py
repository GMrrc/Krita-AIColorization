...
class GetImageFolder(datasets.ImageFolder):   
    ...
    def __getitem__(self, index):
        ...
        color_palette = color_cluster(np.asarray(img), nclusters=self.ncluster)  # Convert back to numpy for clustering
        img = self.make_tensor(np.asarray(img))  # Convert back to tensor after transformations
